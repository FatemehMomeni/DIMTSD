...
# Load the model
model = StanceClassifier(3).to('cuda')
model.load_state_dict(torch.load('../student_models/DIMTSD_seed4.pt'))
# model = torch.load('/kaggle/working/seed4.pt')
model.eval()
...
while flag:
    tweet = input('\nPlease Enter Tweet:')
    target = input('Please Enter Target (in lowercase):')
    domain = input('Please Enter Domain (in lowercase):')
    # Preprocessing
